...
In case you want to only create the spatial characteristics, this is straightforward using `purrr::map()`. 
...
result <- purrr::map(list_pattern, function(x) reconstruct_pattern(pattern = x, 
                                                                   n_random = 3, 
                                                                   verbose = FALSE))
...
```{r result-spatial}
# get mean energy
purrr::map(result, function(x) calculate_energy(pattern = x, 
                                                verbose = FALSE))
```
...
Secondly, to reconstruct the (numeric) marks of the observed pattern for each of the spatially reconstructed patterns, just use `purrr::map()` in combination with `reconstruct_marks()`.
...
result_marks <- purrr::map(reconstructed_pattern, 
...
```{r result-marks}
# get energy
purrr::map(result_marks, function(x) calculate_energy(pattern = x, 
