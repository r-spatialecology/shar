...
reconstruct_spatial <- function(pattern, max_runs = 10000, e_threshold = 0.01, fitting = FALSE){
  ...
  if(fitting==T){ # Fit a Thomas process to the data
    ...
    if(simulated$n != pattern$n){ # Not the same number of points in fitted as in observed data
      dif <- abs(pattern$n -simulated$n)
      if(simulated$n < pattern$n){ # adding random points
        p <- spatstat::runifpoint(n = dif, win = pattern$window)
        simulated <- spatstat::superimpose(simulated, p)
      }
      else{ # removing random points
  ...
  if(pattern$n >= 1000){ # indirect computation because slow spatstat::pcf()
