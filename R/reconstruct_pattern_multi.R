...
#' @param no_changes Integer representing the number of iterations
...
#' @param rmax Maximum distance [m] at which the summary statistics are
#' evaluated.
...
#' @param timing Logical value: The computation timeis measured if this is TRUE
...
#' @param sd This is the standard deviation [m] used in the move_coordinate
#' action.
#' @param steps_tol After the value steps_tol it is checked whether the energy
#' change is smaller than tol.
#' @param tol Tolerance: stops the procedure of energy more than 1 − tol
#' times no_changes.
#' @param w_markcorr Vector of possible weightings of individual mcf's.
...
# Switch selection for the possible changes to the reconstructed point pattern for energy minimisation (probabilities of how often an action is taken: 40%, 10%, 20%, 10%, 10%).
    switch(action,
           ...
           # Swaps the type assignment of two randomly drawn points from the new point pattern, applied in 10% of the trap.
           exchange_mark_two = {
           i                 <- sample.int(nrow(p), 2, replace = FALSE)
           m                 <- p$mark[i, ]
...
# End of reconstruction loop.
  ...
  Parameter_setting <- list(n_repetitions=n_repetitions, max_steps=max_steps,
                            ...
                            show_graphic=show_graphic, Lp=Lp, k=k, bw=bw, sd=sd,
